...
* `download_data.R` includes the necesary code to download the derived (processed) data from Zenodo
* `article/`: Quarto source document for the manuscript. Includes code to reproduce the figures and tables
    generated by the analysis. It also has a rendered version,
    `paper.pdf`, suitable for reading (the code is replaced by figures
    and tables in this file)
    
* `src/` and `R/`: includes the scripts and functions used to pre process the data (CMIP6 model experiments). They
...
Finally, render the main manuscript located at `article/article.qmd` with:
