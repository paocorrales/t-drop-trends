...
```{r}
serie |>
  _[!(year == 2014 & scenario != "historical") & lat < 0] |>
  _[, let(land = MaskLand(lon, lat))] |> 
  _[, let(land = factor(land, labels = c("Sea", "Land")))] |> 
  _[, let(band_lat = cut_width(lat, 5))] |> 
  _[, let(mean_freq = mean(mean_freq)), by = .(land, band_lat, year, scenario)] |> 
  _[, FitLm(mean_freq, year),  by = .(land, band_lat, scenario)] |> 
  _[term == "year"] |> 
  ggplot(aes(scenario, estimate)) +
  geom_point(aes(color = band_lat)) +
  geom_path(aes(color = band_lat)) +
  facet_wrap(~land)
```
