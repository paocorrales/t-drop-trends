...
tas <- purrr::map(Sys.glob(here("data/tas/*hist*mean.nc")), function(f) {
...
tos <- purrr::map(Sys.glob(here("data/tos/*hist*mean.nc")), function(f) {
  ...
  ReadNetCDF(f, vars = c(hist = var)) |> 
    ...
    _[, let(change = sqrt(dt_dx_ssp585^2 + dt_dy_ssp585^2) / sqrt(dt_dx_hist^2 + dt_dy_hist^2),
            diff = sqrt(dt_dx_ssp585^2 + dt_dy_ssp585^2) - sqrt(dt_dx_hist^2 + dt_dy_hist^2))] |> 
    _[, .(time, lat, lon, hist, ssp585, model, var, change, diff)]
...
data <- rbindlist(list(delta, tas, tos)) |> 
  mask[x = _, on = .NATURAL]
...
We calculated the meridional temperature gradient ($\partial T/ \partial y$) at each grid point using central finite differences for the temperature at 850 hPa. We then area-averaged $\partial T/ \partial y$ over different regions, i.e.  between 25$^{\circ}$ and 65$^{\circ}$ S. This process produces the same results as if we calculate the temperature difference between2 latitudes but give us the advantage of calculating the mean $\partial T/ \partial y$ for regions over land and sea independently.

### Storm Tracks estimation

To estimate the storm track location we use the transient eddy kinetic energy (EKE), used by many others to estimate storm tracks [@ogorman2010; @chemke2022]. We calculated EKE using a Fast Fourier Transform high-pass filter applied to daily horizontal wind at 300 hPa, with a 6-day cutoff for each model.  
...
sig <- data |> 
  _[(model %in% models_in_gadi) & var == "deltap97.5"] |>
...
#| fig-width: 8
#| fig-height: 5
#| fig.cap: "Multimodel mean frequency of $\\Delta T$ for the historical period (1850-2014) and SSP5-8.5 (2015-2100) over land and sea for different latitude bands. Left to each main figure the intermodel distribution is shown for the period 2080-2100."
...
serie_regions |> 
  ...
  theme(legend.position = "inside",
        legend.position.inside = c(0.78, 0.93),
...
@fig-eke explores the relationship between the change in the frequency of $\Delta T$ and the response of $\partial T / \partial y$ and EKE by comparing the scenario SSP5-8.5 (2080-2100) and the recent past (1980-2000). There is a positive relationship between the changes in $\Delta T$ and $\partial T / \partial y$ in both hemispheres. The reduction in the frequency of fronts in the Northern Hemisphere could be associated to the decrease of $\partial T / \partial y$ in low levels already shown by other authors [e.g. @catto2014] while the opposite is true for the Southern Hemisphere (@fig-eke a). 
We see a similar relationship between changes in $\Delta T$ and EKE except for the Southern Hemisphere over land. The reason for this it is difference could be associated to many factors. 
The change of the stormtracks in the SH is mainly associated to a poleward shift and may not have an effect on frequency of fronts over land the same way the $\partial T / \partial y$ does. @harvey2014 found that the stormtrack response in the SH during summer is associated to the change in the low level $\partial T / \partial y$ mainly bellow 65$^{\circ}$ S, i.e. over the southern oceans. 
...
change <- data |> 
  ...
  _[, .(mean_eke = weighted.mean(eke, w = cos(lat*pi/180), na.rm = TRUE),
        mean_dt_dy = weighted.mean(dt_dy, w = cos(lat*pi/180), na.rm = TRUE),
        mean_deltat = weighted.mean(deltap97.5, w = cos(lat*pi/180), na.rm = TRUE)), by = .(region, land, model)] |> 
  na.omit()
...
gradt <- change |> 
  ...
  labs(x = latex2exp::TeX("$\\nabla _{y} T$ change (K/km)"), y = latex2exp::TeX("$\\Delta T$ frequency change (days/year)"),
