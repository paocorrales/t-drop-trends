...
keypoints: 
  ...
  - Relationship with temperature gradient?
...
model_spread <- data |> 
  ...
  _[!is.na(serie) & var == "deltat" & model %in% models_in_gadi] |>
...
gradt <- change |> 
  ...
  labs(x = latex2exp::TeX("$\\partial T / \\partial y$ change (K/km)"), y = latex2exp::TeX("$\\Delta T$ frequency change (days/year)"),
...
#   _[, let(land = MaskLand(lon, lat))] |> 
#   _[, let(land = factor(land, labels = c("Sea", "Land")))] |> 
#   _[, .(mean_dt = mean(deltat, na.rm = TRUE)), by = .(lat, land)]
# 
# mean_trend_dt <- data |> 
#   _[(model %in% models_in_gadi) & is.finite(change)] |>
#   dcast(lat + lon + model ~ var, value.var = "diff") |> 
